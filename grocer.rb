...
def apply_coupons(cart, coupons)
  coupons.map do |coupon|
    item = coupon[:item]
    item_with_coupon = cart["#{item} W/COUPON"]
    if cart[item]
      if cart[item][:count] >= coupon[:num] && !item_with_coupon
        item_with_coupon = { price: coupon[:cost] / coupon[:num], clearance: cart[item][:clearance], count: coupon[:num] }
          cart[item][:count] -= coupon[:num]
      elsif cart[item][:count] >= coupon[:num] && item_with_coupon
